...
RUN curl -f "https://get.helm.sh/helm-v${HELM_VERSION}-linux-`cat /arch`.tar.gz" | tar --no-same-owner xzf - -- helm \
	&& mv helm ./bin/helm \
	&& chmod +x ./bin/helm \
	&& helm plugin install https://github.com/databus23/helm-diff

RUN curl -f -L "https://github.com/helmfile/helmfile/releases/download/v${HELMFILE_VERSION}/helmfile_${HELMFILE_VERSION}_linux_`cat /arch`.tar.gz" | tar --no-same-owner xzf - -- helmfile \
...
RUN [[ `arch` == 'x86_64' ]] && normalizedArch="x86_64" || normalizedArch="arm64" \
	&& curl -f -L "https://github.com/derailed/k9s/releases/download/v${K9S_VERSION}/k9s_Linux_${normalizedArch}.tar.gz" | tar --no-same-owner xzf - -- k9s \
